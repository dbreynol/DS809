...
Noise processes (white noise, AR, MA) matter in time-series work because they are the practical language for describing, diagnosing, and modeling uncertainty over time. These are the building blocks of nearly all time series models.
...
Noise offer a precise way of measuring:
...
White noise is a particularly important building block becuase when a time series is white noise, then there is no predictable structure left. That is, a white noise time series will not yield to any amount of forecasting prowess.
...
The random walk is fundamental because it is the simplest model where the past fully determines the present level, yet provides no information about the future—making it the natural benchmark, diagnostic, and cautionary tale in time-series analysis. When a time series follows a random walk, one can estimate the distribution of changes from one time step to the next but not the level of the time series beyond one time step.
