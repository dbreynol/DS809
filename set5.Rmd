...
Now let's implement the MH algorithm and sample from the posterior distribution of $\beta$. Our MCMC for the slope coefficient are summarized below.
...
where $I$ is a $2 \times 2$ identity matrix and $\mathbf{0}$ is a vector with two elements. Use the same simulated data as in the example. Run the algorithm for three different $\lambda$ values (10, 50, 100). For each one, make a trace plot and a histogram for $\beta_1$.
