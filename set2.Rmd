...
1. No systematic change in the mean or the variance
...
1. Change the <code>es</code> function we developed in [Exponential Smoothing] to return the SSE, rather than <code>yt</code>, the smoothed time series. Now, use the <code>optim</code> function to find the values of $(\alpha, l_0)$ that minimize SSE. Plot the exponentially smoothed time series using the optimized values for $(\alpha, l_0)$.
...
3. Theoretical versus simulated autocorrelation.
